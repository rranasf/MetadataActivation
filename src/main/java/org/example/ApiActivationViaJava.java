...
public class ApiActivationViaJava {
    ...
    public static int perPartitionRateLimit = 100;
    ...
    public static HttpRequest getHTTPBatchRequest(final List<String> batchData) {
        // Creating the HTTP Request from partner configs
        String hostName = partnerConfig.connectionProperties.serverAddress;
        int  port = partnerConfig.connectionProperties.serverPort;
        String httpPrefix = partnerConfig.connectionProperties.isSSL ? "https" : "http";
...
//                StringEntity params = new StringEntity(String.join(",", batchData));
...
//                        rateLimiter.acquire(1);
    ...
    public static void processBatch(List<String> batch) {
        ...
        HttpRequest request = getHTTPBatchRequest(batch);
        if (request != null) {
            HttpResponse response = null;
            Callable<HttpResponse> callable = new Callable<HttpResponse>() {
                public HttpResponse call() throws Exception {
            ...
            try {
                ...
                System.out.println("Success: API returned Failure.");
                e.printStackTrace();
            } catch (ExecutionException e) {
                e.printStackTrace();
            }
    ...
    public static void callAPIForRows(Iterator<Row> it) {
...
//        JSONArray jsonArray = new JSONArray();
        ...
        int numberOfMaximumConnections = Integer.valueOf(partnerConfig.connectionProperties.numberOfMaximumConnections);
        numPartitions = numberOfMaximumConnections;

        // Re-partition the Data
        jsonDS.repartition(numberOfMaximumConnections);

        System.out.println("number of partitions After Re-partition: "+ jsonDS.rdd().getNumPartitions());

        int overallRateLimit = Integer.valueOf(partnerConfig.apiProperties.operationalProperties.maxNumberOfRequestsPerMin);
        perPartitionRateLimit = overallRateLimit/numPartitions;
